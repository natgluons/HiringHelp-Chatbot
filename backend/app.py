...
MODEL = os.environ.get('OPENROUTER_MODEL', 'qwen/qwen-2-7b-instruct:free')
...
def chat():
    ...
    try:
        ...
        if "list" in prompt.lower() and "candidate" in prompt.lower():
            ...
            names = set()
            for filename in os.listdir(folder_path):
                if filename.startswith("CV_") and (filename.endswith(".pdf") or filename.endswith(".txt")):
                    # Keep the name exactly as it appears in the filename
                    name = filename[3:-4] if filename.endswith(".pdf") else filename[3:-4]
                    names.add(name)
        ...
        for filename in os.listdir(folder_path):
            if filename.startswith("CV_") and (filename.endswith(".pdf") or filename.endswith(".txt")):
                candidate_name = filename[3:-4]  # Remove "CV_" prefix and extension
from flask import Flask, request, jsonify, send_from_directory
from logging.config import dictConfig
from models import get_db_connection, init_db
import os
